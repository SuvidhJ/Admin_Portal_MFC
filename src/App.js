...
import { FiSearch, FiMail, FiPhone, FiAlertCircle, FiLoader, FiExternalLink } from 'react-icons/fi';
...
const InterviewPortal = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.post('https://recruitment-portal-be-25.onrender.com/admin/response/', {
        regNo: regNo
      });
              ...
              {userData.participatedEvent ? (
                <div className="event-tags">
                  {userData.participatedEvent.split(',').map((event, idx) => (
                    <span key={idx} className="event-tag">{event.trim()}</span>
                  ))}
                </div>
              ) : (
  ...
  const renderDomains = () => {
    if (!userData || !userData.domain || userData.domain.length === 0) return null;
  ...
  const renderDomainAnswers = () => {
    ...
    const filteredTasks = tasks.filter(task => task.tasks && task.tasks.length > 0);
    ...
    return (
      <>
        {filteredTasks.map((domainTask, index) => (
          <div key={index} className={`domain-answers card domain-${domainTask.domain}`}>
            <div className="domain-header">
              <h3>
                {domainTask.domain.charAt(0).toUpperCase() + domainTask.domain.slice(1)} Domain Responses
              </h3>
            </div>
            {domainTask.tasks.map((task, taskIndex) => (
              <div key={taskIndex} className="task-container">
                  ...
                  <div className="subdomain">
                    ...
                    <div className="subdomain-tags">
                      {task.subdomain.flatMap((sub, subIdx) => {
                        if (sub.includes(',')) {
                          return sub.split(',').map((subItem, itemIdx) => {
                            const trimmedItem = subItem.trim();
                            return trimmedItem ? (
                              <span key={`${subIdx}-${itemIdx}`} className="subdomain-tag">
                ...
                {task.question1 && Array.isArray(task.question1) && task.question1.length >= 2 && (
                  <div className="portfolio-section">
                    ...
                    <div className="question-card">
                      ...
                      <div className="answer-container">
                        <p className="answer">{linkifyText(task.question1[1])}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="other-questions">
                  ...
                  <div className="questions-grid">
                    {['question2', 'question3', 'question4', 'question5', 'question6', 'question7', 
                      ...
                      'question14', 'question15', 'question16', 'question17'].map((qKey, qIndex) => (
                      task[qKey] && Array.isArray(task[qKey]) && task[qKey].length >= 2 ? (
        ...
        <div className="search-section">
          <div className="input-group">
            ...
            {regNo && !validateRegNo(regNo) && (
              <div className="validation-hint">Format: YYBXX0000 (YY: 21-24, B: Branch, XX: Dept)</div>
            )}
          </div>
          <button
